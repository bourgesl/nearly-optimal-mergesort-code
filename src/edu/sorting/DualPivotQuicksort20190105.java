 ...
 * @since 1.7 * 12
 */
public final class DualPivotQuicksort20190105 implements wildinter.net.mergesort.Sorter {

    public final static wildinter.net.mergesort.Sorter INSTANCE = new DualPivotQuicksort20190105();

    /**
     * Prevents instantiation.
     */
    private DualPivotQuicksort20190105() {
    }
...
//  private DualPivotQuicksort() {} // TODO
     ...
     */
    private static final int MAX_INSERTION_SORT_SIZE = 45;

    /**
     * Max array size to use pair insertion sort.
     */
    private static final int MAX_PAIR_INSERTION_SORT_SIZE = 181;

    /**
     * Max array size to use heap sort for the leftmost part.
     */
    private static final int MAX_HEAP_SORT_SIZE = 15;
    ...
    private static void sort(Sorter sorter, int[] a, int bits, int low, int high) {
        while (true) {
            ...
            if (size < MAX_PAIR_INSERTION_SORT_SIZE && (bits & 1) > 0) {
                pairInsertionSort(a, low, high - 3 * (size >> 4 << 2), high);
                return;
            }
             ...
             */
            if (size < MAX_INSERTION_SORT_SIZE && bits < 4) {
...
//          if (a[k] < a[k - 1]) {    // TODO [3a]     check on 50
//          int ak = a[i = k];
            int ak = a[i = k];
//          if (ak < a[i - 1]) {      // TODO [3i]
            if (ak < a[k - 1]) {      // TODO [3k]
...
//              a[i + 1] = ak;
    ...
    private static void pairInsertionSort(int[] a, int left, int end, int last) {
        ...
        for (int i, r = last; ++left < end;) {        // TODO: i <-> k   or ak <-> ai
            int ak = a[i = left];

            if (ak < a[i - 1] || ak > pivot) {         // [p]

                if (ak > pivot) {

                    while (i < r && a[--r] > pivot);   // [i]
        ...
        for (int i; left < last; ++left) {             // TODO: i <-> k
            int a1 = a[i = left], a2 = a[++left];

            if (a1 > a2) {
//          if (a1 > a2 || a2 < a[i - 1]) { // TODO [1] ? x
//          if (a2 < a[i - 1]) { // TODO [1.1] ? x
