 ...
 * @since 1.7 * 12
 */
public final class DualPivotQuicksort20181127Ext implements wildinter.net.mergesort.Sorter {

    private final static boolean DO_CHECK = false;

    public final static wildinter.net.mergesort.Sorter INSTANCE = new DualPivotQuicksort20181127Ext();

    /**
     * Prevents instantiation.
     */
    private DualPivotQuicksort20181127Ext() {
    }
...
//  private DualPivotQuicksort() {} // TODO
    ...
    static void sort(Sorter sorter, int[] a, int[] b, int bits, int low, int high) {
        while (true) {
            ...
            if (size < MAX_PAIR_INSERTION_SORT_SIZE && (bits & 1) > 0) {
                pairInsertionSort(a, b, low, high - 3 * (size >> 4 << 2), high);
                return;
            }
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    private static void pairInsertionSort(int[] a, int[] b, int low, int end, int last) {
        /*
