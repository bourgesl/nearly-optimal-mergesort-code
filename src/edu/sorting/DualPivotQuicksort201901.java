 ...
 * @since 1.7 * 12
 */
public final class DualPivotQuicksort201901 implements wildinter.net.mergesort.Sorter {

    public final static wildinter.net.mergesort.Sorter INSTANCE = new DualPivotQuicksort201901();

    /**
     * Prevents instantiation.
     */
    private DualPivotQuicksort201901() {
    }
...
*/
    ...
    private static void sort(Sorter sorter, int[] a, int bits, int low, int high) {
        while (true) {
             ...
             */
            if (size < MAX_PAIR_INSERTION_SORT_SIZE) {
                hybridInsertionSort(a, low, high - 3 * (size >> 4 << 2), high);
                return;
            }
...
*/
    ...
    private static void hybridInsertionSort(final int[] a, final int low, final int end, final int last) {
        // end = last (high), if array is too small, pair insertion sort is not invoked

        // end = left + size / 4, if arrays is big enough for pair insertion sort.
        //    1/4 of array will be sorted by insertion sort, other 3/4 will be sorted by
        // pair insertion sort
        int left = low;

        // Start insertion sort on tiny part
        for (int i, r = last, pivot = a[end - 1]; ++left < end; ) {
            int ak = a[i = left];

            if (ak < a[i - 1] || ak > pivot) {
                if (ak > pivot) { // additional check
                    while (i < r && a[--r] > pivot);
        ...
        for (int i; left < last; ++left) {
            int a1 = a[i = left], a2 = a[++left];
