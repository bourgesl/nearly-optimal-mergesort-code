...
public final class DualPivotQuicksort202105InPlace implements wildinter.net.mergesort.Sorter {

    private final static boolean TRACE = false;
    private final static boolean TRACE_ALLOC = false;

    /* sort algorithm bit masks */
    public final static int SORT_DPQS = 1; // Dual-Pivot Quicksort in-place
    public final static int SORT_TRY_MERGE = 2; // merge pre-sorted runs
    public final static int SORT_RADIX = 4; // use radix-sort
    public final static int SORT_ALL = SORT_DPQS | SORT_TRY_MERGE | SORT_RADIX; // all (hybrid sort)

    public final static wildinter.net.mergesort.Sorter HYBRID = new DualPivotQuicksort202105InPlace(SORT_ALL);
    public final static wildinter.net.mergesort.Sorter DPQS_ONLY = new DualPivotQuicksort202105InPlace(SORT_DPQS);
    public final static wildinter.net.mergesort.Sorter RADIX_ONLY = new wildinter.net.mergesort.Sorter() {

        // avoid alloc
        private final DualPivotQuicksort202105InPlace INSTANCE = new DualPivotQuicksort202105InPlace(SORT_RADIX);
        ...
        public void sort(final int[] A, final int low, final int high) {
            ...
            DualPivotQuicksort202105InPlace.radixSort(INSTANCE.sorter, A, low, high + 1); // exclusive
        }
    };
     ...
     */
    private DualPivotQuicksort202105InPlace(final int sortMask) {
        sorter = new Sorter(sortMask);
    }
    ...
    static void sort(Sorter sorter, int[] a, int bits, int low, int high) {
        while (true) {
            ...
            if (a[e1] < a[e2] && a[e2] < a[e3] && a[e3] < a[e4] && a[e4] < a[e5]) {
                ...
                if (((sorter.sortMask & SORT_RADIX) != 0) && (true /* || sorter == null*/ || bits > MIN_RADIX_SORT_DEPTH) && size > MIN_RADIX_SORT_SIZE) {
...
//                  System.out.println("alloc run (resize)");
        ...
        if (count > 1) {
            ...
            if ((sorter.sortMask & SORT_TRY_MERGE) == 0) {
                return false;
            }
...
//        }
    ...
    static final class Sorter {
        ...
        void initBuffers(final int length, final int offset) {
            if ((sortMask != SORT_DPQS) && ((b == null) || (b.length < length))) {
                if (TRACE_ALLOC) {
